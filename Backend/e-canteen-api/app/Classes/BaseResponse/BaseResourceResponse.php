...
class BaseResourceResponse implements Responsable
{
    /**
    ...
    protected function wrap($data, $with = [], $additional = [])
    {
        ...
        if (!$this->resource instanceof BaseCollection) {
            ...
            return array_merge_recursive($with, $data, $additional);
        } else {
            if ($this->haveDefaultWrapperAndDataIsUnwrapped($data)) {
                $data = [$this->wrapper() => ['data' => $data]];
            } elseif ($this->haveAdditionalInformationAndDataIsUnwrapped($data, $with, $additional)) {
            ...
            if(isset($with['data'])) {
                $attr['data'] = $with['data'];
                $data = array_merge_recursive($data, $attr);
                unset($with['data']);
            }
        }
        return array_merge_recursive($with, $data, $additional);
